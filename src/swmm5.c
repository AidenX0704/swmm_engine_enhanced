...
char *getCsvFilePath(const char *path) {
    ...
    // 如果没找到后缀，那就直接添加.csv后缀
    if (dotPos == 0) {
        char *newPath = (char *)malloc((pathLen + 5) * sizeof(char));
        if (newPath == NULL) {
            return NULL;
        }
        strcpy(newPath, path);
    ...
    // 找到了后缀，判断是否已经是.csv相关后缀（不区分大小写）
    int isCsvSuffix = 1;
    const char *csvSuffix = ".csv";
    size_t csvLen = strlen(csvSuffix);
    if (pathLen - dotPos == csvLen) {
        for (size_t i = 0; i < csvLen; ++i) {
            if (tolower(path[dotPos + i])!= csvSuffix[i]) {
                isCsvSuffix = 0;
                break;
            }
        }
    ...
    // 如果不是.csv后缀，就替换为.csv后缀
    if (!isCsvSuffix) {
        char *newPath = (char *)malloc(pathLen - (pathLen - dotPos) + 5);
        if (newPath == NULL) {
            return NULL;
        }
        strncpy(newPath, path, dotPos);
        newPath[dotPos] = '\0';
        strcat(newPath, ".csv");
        return newPath;
    }

    // 如果已经是.csv后缀了，直接返回原路径的拷贝
    char *newPath = (char *)malloc((pathLen + 1) * sizeof(char));
    if (newPath == NULL) {
        return NULL;
    }
    strcpy(newPath, path);
...
//
{
    ...
    // --- Prepare the CSV file for timeseriesFilePath
    char timeseriesFilePath[512];
    snprintf(timeseriesFilePath, sizeof(timeseriesFilePath), "%s", getCsvFilePath(f1));
    FILE *timeseriesFile = fopen(timeseriesFilePath, "w");
    if (timeseriesFile == NULL)
    {
        writecon("\n o  Error opening timeseries CSV file");
    ...
    // --- report results and out time series
    ...
    for (j = 0; j < Nobjects[NODE]; j++)
    {
        ...
        if (k >= 0)
        {
            for (period = 1; period <= Nperiods; period++)
            {
                ...
                // 获取节点的坐标
                fprintf(timeseriesFile, "%s %s,%s,%9.3f\n", theDate, theTime, Node[j].ID, (NodeResults[NODE_OVERFLOW] * 60.0));
            }
        }
    }
